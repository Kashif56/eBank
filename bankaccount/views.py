...
def generate_account_number():
    ...
    credit_card_number = '4141' + remaining_digits
...
def verifyAccountNumber(request, account_number):
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        bank_account = BankAccount.objects.filter(account_number=account_number)
        if bank_account.exists():
            account = bank_account.first()
            data = {
                'success': True,
                'account_data': {
